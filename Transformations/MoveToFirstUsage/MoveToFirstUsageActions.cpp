//===-- MoveToFirstUsage/MoveToFirstUsageActions.cpp - Matcher callback ------------------===//
//
...
///
/// \file
/// \brief This file contains the definition of the MoveToFirstUsageFixer class which is
...
MoveToFirstUsageFixer::MoveToFirstUsageFixer(unsigned &AcceptedChanges,
                           Transform &Owner)
    : AcceptedChanges(AcceptedChanges), Owner(Owner) {
}


void MoveToFirstUsageFixer::run(const ast_matchers::MatchFinder::MatchResult &Result) {
  using namespace std;
  ASTContext& context = *Result.Context;
  SourceManager& SM = context.getSourceManager();

  const auto* node = Result.Nodes.getNodeAs<BinaryOperator>(MatcherMoveToFirstUsageID);
  if ( node ) {
